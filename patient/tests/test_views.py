...
class PatientDetailViewTest(TestCase):
    ...
    def test_authentication_required(self):
        ...
        self.assertRedirects(
            response, f"/login/?next=/patient/{self.patient.id}/detail/"
        )
...
class PatientCreateViewTest(TestCase):
    ...
    def test_form_submission_valid_data(self):
        ...
        self.assertTrue(
            Patient.objects.filter(first_name="Juzef", last_name="Patient").exists()
        )
...
class PatientUpdateViewTest(TestCase):
    ...
    def test_authentication_required(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, f"/login/?next=/patient/{self.patient.id}/update/"
        )
    ...
    def test_form_submission(self):
        ...
        response = self.client.post(
            reverse("patient:update", args=[self.patient.id]), data=data
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, reverse("patient:detail", args=[self.patient.id])
        )
...
class PatientDeleteViewTest(TestCase):
    ...
    def test_authentication_required(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, f"/login/?next=/patient/{self.patient.id}/delete/"
        )
